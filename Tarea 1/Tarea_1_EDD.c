...
char **buscar_str(char **S, int n, char *P, int *m)
{
    ...
    for (int i = 0; i < n; i++)
    {                  //Iteramos sobre cada palabra que contiene S
        int igual = 0; //igual es un flag que nos dice que si es =0, p es prefijo de S[i], y si es =1 P no es prefijo de S[i]
        for (int y = 0; y < largo_P; y++)
        { //Iteramos sobre los caracteres, tanto de P como de S[i]
            if (P[y] == '\n' || ((P[y]) != (S[i][y])))
            {
                igual = 1;
                break;
 ...
 * *******
 * Resumen:
    * La función main, cuenta la cantidad de lineas del archivo S.txt (nlineas) para luego asignarle memoria
    * dinamica y luego guardar todos los string en el array palabras[], ahora lo mismo se hace con el archivo 
    * P.txt, al cual se le cuenta la cantidad de lineas (nprefijos) para luego asignarle memoria dinamica y 
    * guardar los string en el array lista_prefijos[], todo esto lo hacemos para leer los string y contar 
    * sus lineas que nos serviran como parametro en el ciclo for que aplicara la funcion buscar_str en cada 
    * string de P para obtener todas la palabras que esten en S.txt, por cada prefijo que este en P.txt y luego 
    * crear un cuadro de texto con la salida (.out) de todos los string que se hayan obtenido de dicho prefijo.
 * *******
 * Input:
    * S.txt: Texto que contiene los string en forma columnar (con \n entre palabras), las cuales 
    * seran los string que nos debe retornar luego con los prefijos.out
    * P.txt: Texto que contiene los string en forma columnar (con \n entre palabras), las cuales
    * seran las que recorreran los string en el texto S.txt
 * Return:
    * lista_prefijos.out: Habra tantos textos como nprefijos haya en P.txt, textos en los cuales contendran
    * los string que concidan con el prefijo dado en el S.txt
 * ****/
