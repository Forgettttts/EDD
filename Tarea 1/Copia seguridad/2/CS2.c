...
char **buscar_str(char **S, int n, char* P, int* m){
    ...
    for(i=0;i<n;i++){  //iremos viendo string por string de S
        ...
        for (y=0; y<largo_P;y++){ //iremos viendo letra por letra tanto de P como se S[i]
            //* Prueba: printf("Letra de P actual= %c\n", P[y]);
            if ((P[y])!=(S[i][y])){
                igual=1;
                break;
            }
        ...
        if (igual == 0){
            printf("El string %s tiene como prefijo a el string %s\n", S[i], P);
        }
        
        if (igual==0){
            strcpy(Ret[tamRet], S[i]); //Ret[tamRet]=S[i];
                tamRet;
        }

        //si es que el igual=0, añadimos la palabra a Ret, si es igual=1, no se agrega
...
int main()
{
    ...
    int m=0;
    respuesta= buscar_str(palabras, nlineas, lista_prefijos[0], &m );
